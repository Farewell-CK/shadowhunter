...
针对超大监控视频（如 10 小时以上的录像），系统摒弃了固定时长切片（8s/段）的传统做法：
- **原理**：系统先对全视频进行 1fps 的极速“流式粗扫”，记录下目标出现的所有离散时间点。
- **动态聚类**：将相邻的时间点自动聚合成一个完整的“案发事件块”，并前后预留冗余时间。
- **解决痛点**：彻底解决了目标出现在固定切片边缘（如 7.9s 出现，8.1s 消失）导致的分析冗余或目标丢失，实现“以人为中心”的动态定点截取。
